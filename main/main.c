/*
 * mac_monitor — Mac 系统状态四圆环仪表（CPU / GPU / 内存 / 风扇·温度）
 * 板级初始化基于 Waveshare 02_ESP_IDF_ST7789_LVGL 例程。
 */
...
static const char *TAG = "mac_monitor";
...
static void app_touch_init(void)
{
...
void app_main(void)
{
    ...
    /* Build the 4-ring gauge UI */
    lvgl_port_lock(-1);
    ui_init();
    lvgl_port_unlock();

    /* Start reading stats lines from USB serial */
    serial_input_start();

    /* WiFi + UDP stats channel (primary transport) */
    wifi_sta_init();
